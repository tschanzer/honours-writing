...
\epigraphhead[0.1\textheight]{
    \epigraph{
        ...
        \hfill\begin{minipage}[b]{\dimexpr\textwidth-1cm}
            When shall we three meet again \\
            In thunder, lightning, or in rain?
        \end{minipage}
    }{
        Shakespeare, \emph{Macbeth}, Act I, Scene I
    }
}

\todo{introductory paragraph with motivation (from proposal) and outline}
...
\parencite{zacharuk2018,alcala2021}. \todo{fix last sentence}
...
\todo{x = <z>, motivate data-driven method (see update presentation)}
...
adjacent grid points to match the resolution of $\vec{x}$ \todo{not
necessarily!}. Now, for each state $\vec{z}$ in the training dataset, one may
